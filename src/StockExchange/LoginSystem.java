...
public class LoginSystem {
	...
	private ArrayList<ExchangeUser> users = new ArrayList<>();
		...
		while (!signedIn) {
			...
			if (choice == 1) {
				...
				for (ExchangeUser user : users) {
					if (user.getApproved() && user.getUsername().equals(userName)
							&& user.getPassword().equals(password)) {
						return user;
					}else if(!user.getApproved() && user.getUsername().equals(userName) && user.getPassword().equals(password)) {
				...
				while (true) {
					...
					while (true) {
						...
						if (choice == 1) {
							...
							ExchangePlatformOperator temp = new ExchangePlatformOperator(userName, password, name);
							System.out.println("Exchange Platform Operator added");
							users.add(temp);
							break;
						} else
	...
	public void loadTestScenario(ExchangePlatform ep) {
		...
		Order order1 = new Order(ep.getNewOrder(), security1.getSI(), 2.2f, 10, Utils.getTime(), Type.sell, trader1);
		Order order2 = new Order(ep.getNewOrder(), security2.getSI(), 1.3f, 10, Utils.getTime(), Type.purchase,
				trader1);
		Order order3 = new Order(ep.getNewOrder(), security1.getSI(), 2.4f, 5, Utils.getTime(), Type.sell, trader2);
		Order order4 = new Order(ep.getNewOrder(), security2.getSI(), 1f, 5, Utils.getTime(), Type.purchase, trader2);

		Order order5 = new Order(ep.getNewOrder(), security3.getSI(), 3.2f, 10, Utils.getTime(), Type.purchase,
				trader1);
		Order order6 = new Order(ep.getNewOrder(), security4.getSI(), 5.1f, 10, Utils.getTime(), Type.sell, trader1);
		Order order7 = new Order(ep.getNewOrder(), security3.getSI(), 2.9f, 5, Utils.getTime(), Type.sell, trader2);
		Order order8 = new Order(ep.getNewOrder(), security4.getSI(), 4.9f, 5, Utils.getTime(), Type.purchase,
				trader2);
	...
	public boolean addUser(ExchangeUser user) {
